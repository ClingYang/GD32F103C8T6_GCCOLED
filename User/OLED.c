...
{
	/*根据BitValue的值，将PB8（SCL）置高电平或者低电平*/

	gpio_bit_write(I2C_SOFT_PORT, I2C_SOFT_SCL_PIN, (bit_status)BitValue);
...
{
	/*根据BitValue的值，将PB9（SDA）置高电平或者低电平*/
	gpio_bit_write(I2C_SOFT_PORT, I2C_SOFT_SDA_PIN, (bit_status)BitValue);
...
void OLED_Update(void)
{
	...
	for (j = 0; j < 8; j++)
	{
		/*设置光标位置为每一页的第一列*/
		OLED_SetCursor(j, 0);
		/*连续写入132个数据，将显存数组的数据写入到OLED硬件*/
		OLED_WriteData(OLED_DisplayBuf[j], 132);
	}
}
